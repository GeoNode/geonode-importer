...
class TestShapeFileFileHandler(TestCase):
    ...
    def test_import_with_ogr2ogr_without_errors_should_call_the_right_command(
        ...
        _open.assert_called_with(
            f'/usr/bin/ogr2ogr --config PG_USE_COPY YES -f PostgreSQL PG:" dbname=\'test_geonode_data\' host=localhost port=5434 user=\'geonode_data\' password=\'geonode_data\' " "{self.valid_shp.get("base_file")}" -lco DIM=2 -nln alternate "dataset" -lco precision=no -lco GEOMETRY_NAME=geometry ',
            stdout=-1,
            stderr=-1,
            shell=True,  # noqa
        )
